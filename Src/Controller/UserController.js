...
const UserController = {
    ...
    async verifyEmail(req, res, next) {
        try {
            ...
            if (!user) {
                return next(
                    ErrorHandler.unAuthorized('Invalid Verification Link')
                );
            }
            if (user.verified) {
                return next(
                    ErrorHandler.unAuthorized('User Is Already Verified')
                );
            }
    ...
    async getAllUserDetails(req, res, next) {
        try {
            /* While searching user dont show current user record */
            ...
            const keyword = req.query.search
                ? {
                      $and: [
                          ...
                          { _id: { $ne: authUserId } }, // Exclude the authenticated user's record
                      ],
                  }
                : { _id: { $ne: authUserId } };
    ...
    async updateUserRole(req, res, next) {
        ...
        try {
            ...
            if (userData.name !== req.body.name) {
                return next(
                    ErrorHandler.Restricted("You Can't Change The User Name")
                );
            }
            if (userData.email !== req.body.email) {
                return next(
                    ErrorHandler.Restricted("You Can't Change The User Email")
                );
            }
    ...
    async editUserprofile(req, res, next) {
        try {
            ...
            if (req.body.email) {
                ...
                if (userEmailCheck) {
                    return next(
                        ErrorHandler.unAuthorized('This email is already taken')
                    );
                }
            }
            const newUserData = {
